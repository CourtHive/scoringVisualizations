...
export function gameTree() {
  ...
  function selectView(_, i: number, self: any) {
    ...
    if (select('[id=' + selector + ']').attr('status') == 'none') {
      if (select(self).attr('selector') == 'Opponent') {
        select('[id=Player]').attr('opacity', 0.4).attr('status', 'none').attr('fill', options.nodes.colors.neutral);
        select('[id=Opponent]').attr('opacity', 1).attr('status', 'selected').attr('fill', options.nodes.colors[i]);
